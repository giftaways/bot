...
export const handleClientReady = async (client: ExtendedClient) => {
    ...
    var globalCommands = Array.from(client.commands.filter(x => x.global === true).values()).map(x => x.options.toJSON()) as RESTPostAPIApplicationCommandsJSONBody[];

    const applicationCommands = await rest.put(Routes.applicationCommands(client.user?.id as string), { body: globalCommands }).catch(console.error) as ApplicationCommand[];
